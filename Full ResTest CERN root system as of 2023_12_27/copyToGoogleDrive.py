...
def copyPlotsToDir(from_dir:str, to_dir:str, delete_preexisting_plots:bool, dp)->None:
    ...
    if delete_preexisting_plots:
        dp.debug(3,f"Deleting contents of {to_dir} before moving new files into it.") 
        os.system(f"rm {to_dir}/*.png 2>/dev/null")
    dp.debug(4,f"Copying png files from {to_dir} to {to_dir}.") 
    os.system(f"cp {from_dir}/*.png {to_dir}/")
    dp.debug(4,f"finished copying.",1) 
...
def copy_plotsdir(tomlData) -> None:
    ...
    if copy_to_google_drive:
        ...
        if all_ok:
            ...
            db.debug(1,"Error, config errors prevent copy_overlays")
...
def copy_scores(tomlData) -> None:
    ...
    if copy_to_google_drive:
        ...
        if all_ok:
            subprocess.run(["cp", score_file, score_file_GDrive])
        else:
            db.debug(1,"Error, config errors prevent copy_overlays")
...
def copyToGoogleDrive_data(tomlData) -> None:
    ...
    if copy_to_google_drive and all_ok:
        ...
        db.debug(3,f"copy_to_google_drive = false in {cfg_file_name}. No copy to Google Drive was performed.")
