...
TCanvas * newTCanvas(char* rootname, char* title = const_cast<char*>("")); //makes an 800x800 TCanvas and cd's into it. 
...
void PrettyFonts(TH1F* h){
        ...
        y->SetTitleFont(132);
        x->SetTitleSize(0.06);
        x->SetTitleOffset(1.71); //make the Title a little further from the axis
        y->SetTitleOffset(0.82);
        y->SetTitleSize(0.06);
        x->SetLabelFont(132);
        y->SetLabelFont(132);
        x->SetLabelSize(0.04);
...
TCanvas * newTCanvas(char* rootname, char* title, int x = 800, int y = 800){
